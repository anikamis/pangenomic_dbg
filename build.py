...
def main():
    ...
    write_json(nodes, strains, links, outfix)
    print(len(nodes), len(links))

#def get_unique_seqs(graph, strainmap, colors)
...
def generate_colors(strains):
    ...
    for strain in strains:
        colors[strain] = "#%06x" % random.randint(0, 0xFFFFFF)
...
def nodes_to_json(graph, colors, k, maxlen):
    ...
    scale = lambda x : int((x - k)/(maxlen - k) * (100 - 4) + 5)
    ...
    for label, node in graph.nodes.items():
        ...
        nodes.append(temp)
    return sorted(nodes, key=lambda x: len(x["strains"]))
    return nodes
...
def get_non_branching(graph):
    ...
    for n_label, n in graph.nodes.items():
        if n.is_branching():
            start = n.copy_in() if n_label not in compress.nodes else compress.nodes[n_label]
            ...
            for out_edge, out_node in n.outgoing.items():
                ...
                end = curr_end.copy_out() if curr_end.label not in compress.nodes else compress.nodes[curr_end.label]
                ...
                if count == 0:
                    ...
                    maxlen = len(middle.label) if len(middle.label) > maxlen else maxlen
...
def build_graph_from_seq(strain, seq, k, graph=None):
    ...
    for i in range(len(seq) - k + 1):
        ...
        if in_mer not in nodes: nodes[in_mer] = N(in_mer)
        if out_mer not in nodes: nodes[out_mer] = N(out_mer)
